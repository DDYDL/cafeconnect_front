...
function InsertMainStore() {
  ...
  const handleCheckPwd = () => {
    if (pwdRef.current.value != pwdCheckRef.current.value) {
      ...
      setSamePwd(true);
      setNotSamePwd(false);
      pwdRef.current.setAttribute("readonly", true);
      pwdCheckRef.current.setAttribute("readonly", true);
    }
  };
  ...
  const checkId = async () => {
    try {
      const response = await axios.post(
        `http://localhost:8080/main/checkId`,
        form
      );
      if (response.data.state == "success") {
        ...
        setExistId(false);
        setAbleJoin(true);
        idRef.current.setAttribute("readonly", true);
      }
    } catch (error) {
      console.log("error!");
      setError(error);
  ...
  const joinMember = async () => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:8080/main/insert`,
        form
      );
      alert("회원가입 성공");
          ...
          <div className={`${styles["InsertMainStore"]} ${styles["screen"]}`}>
            <div className={styles["background"]}>
              ...
              <div className={styles["overlap-group"]}>
                <div
                  className={`${styles["text-4"]} ${styles["valign-text-middle"]}`}
                >
                  계정 등록
                </div>
                <div className={styles["small-btn_brown"]} onClick={joinMember}>
                  <div
                    className={`${styles["text-5"]} ${styles["valign-text-middle"]} ${styles["themewagongithubiosemanticheading-6"]}`}
                  >
                    계정등록
                  </div>
                </div>
                ...
                <div className={`${styles["flex-row"]} ${styles["flex"]}`}>
                  <div className={`${styles["flex-col"]} ${styles["flex"]}`}>
                    <div className={styles["overlap-group-1"]}>
                      ...
                      <p
                        className={`${styles["text-14"]} ${styles["valign-text-middle"]} ${styles["notosanskr-light-coconut-12px"]}`}
                      >
